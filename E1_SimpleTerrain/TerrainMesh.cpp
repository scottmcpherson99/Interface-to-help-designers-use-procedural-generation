...
void TerrainMesh::Regenerate( ID3D11Device * device, ID3D11DeviceContext * deviceContext ) {
	...
	//run the correct algorithm based on what value has been given as true based on user interactions
	if (smooth == true)
	{
		BuildSmoothMap();
	}
	...
	{
		BuildFault(faultNumbers, faultValue);
	}
	...
	{
		BuildParticleDeposition(particlesDropped_, particleDropX_, particleDropY_, particleDepositionIterations);
	}
	else
	{
		BuildHeightMap();
...
//Run an algorithm to reset the terrain to its default flat value
void TerrainMesh::Flat()
{
	//set every height to 0
	for (int j = 0; j < (resolution); j++) {
		for (int i = 0; i < (resolution); i++) {
			heightMap[j * resolution + i] = 0;
		}
...
void TerrainMesh::BuildFault(const int totalFaults, float faultValue)
{
		int edge[3];
		int edge2[3];
		int point_[2];
		...
		{	//pick two points on the edge of the terrain
			...
			for (int j = 0; j < (resolution); j++) {
				for (int i = 0; i < (resolution); i++) {
					...
					//fault the terrain, raising or lowering the point based on the side of the terrain it belongs to
					if (new_value[1] >= 0)
					{
						if (counter == 0)
						{
							heightMap[(j * resolution) + i] = heightMap[(j * resolution) + i] + faultValue;
						}
						else if (counter == 1)
						{
							heightMap[(j * resolution) + i] = heightMap[(j * resolution) + i] - faultValue;
			...
			faultValue = faultValue * 0.75;
		}
	
	}

//Smooth the terrain to blend it together
void TerrainMesh::BuildSmoothMap()
{
	float height = 0.0f;
	int counter = 0;
	//Scale everything so that the look is consistent across terrain resolutions
...
void TerrainMesh::BuildMidPointDisplacement()
{
		//declare variables
		int min_corner_X = 0;
...
void TerrainMesh::BuildVoronoiRegions(const int voronoiRegions)
{
	...
	//choose the locations of the dropped voronoi seeds
	...
	//run a loop for every point on the terrain to check which voronoi seed the point is closest to and assign it to that region
	for (int j = 0; j < (resolution); j++) {
		for (int i = 0; i < (resolution); i++) {
			distance = 1000;
...
void TerrainMesh::BuildParticleDeposition(int particlesDropped, int particleDropX, int particleDropY, int iterations)
{
